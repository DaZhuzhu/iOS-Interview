...
### 5、class![isa_superclass](/Users/xiaozhuzhu/Documents/work/iOS资料/image/isa_superclass.png)
...
![class_struct](/Users/xiaozhuzhu/Documents/work/iOS资料/image/class_struct.png)

**isa**：一个指针，存储Class和Meta-class的内存地址
...
tcp面向连接，传输数据必须是在持续连接的基础上。有三次握手。

比较慢，但是可靠。

Udp不面向连接，传输数据前是不与对方建立连接的，对接收到的数据也不发送确认通知。
...
1、客户端向服务端发送syn包，进入SYN_SEND状态

2、服务端收到客户端的syn包后，确认客户端的syn包，并给客户端发送一个syn+ack包，进入SYN_RECEIVE状态

3、客户端收到服务端的syn+ack包后，会再次向服务端发送一个ack包（确认包），发送完毕后，客户端和服务端就进入了ESTABLISHED状态（已连接状态）
